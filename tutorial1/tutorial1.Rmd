...
\end{align*}
  ...
  - **Second stage**: use that equation to predict $X_1$, getting $\hat{X_1}$. Then, use those predictions to predict $Y$
...
- $Z_1$ is a pretty effective instrument even if the correlation between $Z_1$ and $X_1$ is small
  - Check relevance: $\text{cov} (X_1, Z_1) =$  `r round(cor(df$x1, df$z1), 3)`, so it's a weak instrument
  - Check validity: $\text{cov} (Z_1, U_1) =$ `r round(cor(df$z1, df$u1), 3)`, pretty close to zero
...
- Why? Because of the weak instrument problem
  - Check relevance: $\text{cov} (X_2, Z_2) =$ `r round(cor(df$x2, df$z2), 3)`
  - Check validity: $\text{cov} (Z_2, U_2) =$ `r round(cor(df$z2, df$u2), 3)`
...
- Even though the correlation between $Z$ and $U$ is the same as previously, the strength of the instrument in explaining $X$ wins out and gives us a better estimator than OLS
  - Check relevance: $\text{cov} (X_3, Z_2) =$ `r round(cor(df$x3, df$z2), 3)`
  - Check validity: $\text{cov} (Z_2, U_2) =$ `r round(cor(df$z2, df$u2), 3)`
...
- The $X$ coefficient estimate of `r round(qr[["coefficients"]][["x"]], 3)` says "one unit increase in $X$ is associated with `r round(qr[["coefficients"]][["x"]], 3)` increase in the $90^{\text{th}}$ quantile of $Y$"
...
Slides
- Huntington-Klein, N. Econometrics Course Slides, [Week 8: Instrumental Variables](https://github.com/NickCH-K/EconometricsSlides/blob/master/Week_08/Week_08_Instrumental_Variables.html)
